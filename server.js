	...
	pool.getConnection(function(err, connection) {
		...
		connection.query('SELECT * FROM `Users` ORDER BY userID', function (error, results, fields) {
			...
			for ( i = 0; i < ln; i++ ){
				...
				logTime = moment.tz(users[i].timeZone).format('DD/MM/YYYY HH:mm');
...
function ArdRecv(userId,time){
	...
	pool.getConnection(function(err, connection) {
		...
		http_request.post("http://aquatest-testfeed.rhcloud.com/api/worker", {form: {key: userId}, headers: headers}, function(err){
			if (err) {
				return;
			}
		}); 
